...
module.exports={
    ...
    addImageProcess:(myImg, path)=>{
        return new Promise((resolve)=>{
            ...
            myImg.save(function(err,image){
                if(err) resolve(400)
                resolve({message:'Image posted !',image})
            })
        })
    },
    ...
    showImageProcess:(id,key)=>{
        return new Promise((resolve)=>{
            colImage.findOne({_id: id},(err, image)=> {
                if (!image) resolve(404)
                if (err) resolve(400)
                else{
                    var params = { 
                        Bucket: nameBucket,
